...
void CNSSolver::BC_HeatFlux_Blowing(CGeometry* geometry, CSolver**, CNumerics*, CNumerics*, CConfig* config,
                                    unsigned short val_marker) {
...
void CNSSolver::BC_HeatFlux_Wall_Generic_Blowing(const CGeometry* geometry, const CConfig* config,
  ...
  if (kind_boundary == BLOWING) {
    ...
    // config->GetTemperature_Ref()/config->GetHeat_Flux_Ref(); Tinfinity =
    // config->GetWall_HeatTransfer_Temperature(Marker_Tag)/config->GetTemperature_Ref();
  }
  ...
  for (auto iVertex = 0u; iVertex < geometry->nVertex[val_marker]; iVertex++) {
    ...
    if ((kind_boundary == BLOWING) && blowingAsMovingGrid) {
      ...
      AddDynamicGridResidualContribution(iPoint, Point_Normal, geometry, UnitNormal, Area, &blowveln, Jacobian_i,
                                         Res_Conv, Res_Visc);
    }
